...
def run_background_tasks():
    start_ts = time.time()
    LOG.info(
        "periodic_tasks->run_background_tasks:Starting background tasks start")
    ...
    pipeline_run_obj.save()
    LOG.info("periodic_tasks->run_background_tasks: background tasks end")
    save_execution_time(start_ts, "run_background_tasks")


def aggregate_data(wkld_results):
    LOG.info("periodic_tasks->aggre_data:aggregate_data task started")
    ...
    LOG.info("periodic_tasks->aggre_data:aggregate_data task end")
    return knob_data, metric_data


def run_workload_characterization(metric_data):
    LOG.info(
        "periodic_tasks->workload_characterization:workload_characterization task started"
    )
    ...
    LOG.info(
        "periodic_tasks->workload_characterization:workload_characterization task end"
    )
...
def run_knob_identification(knob_data, metric_data, dbms):
    LOG.info(
        "periodic_tasks->knob_identification:workload_characterization task start"
    )
    ...
    LOG.info(
        "periodic_tasks->knob_identification:workload_characterization task end"
    )
